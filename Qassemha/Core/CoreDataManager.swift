...
class CoreDataManager: ObservableObject {
    ...
    /// Persists any pending changes in CoreData context to disk
    /// Prevents data loss by ensuring all modifications are saved immediately
    func save() {
    ...
    /// Stores receipt data in UserDefaults as JSON, updating existing or adding new
    /// Uses UserDefaults instead of CoreData for faster implementation and easier migration
    ...
    /// Retrieves all saved receipts from UserDefaults sorted by date (newest first)
    /// Provides access to complete receipt history for display in history view
    ...
    /// Fetches only the most recent receipts up to specified limit for performance
    /// Optimizes home screen loading by limiting data retrieval to what's visible
    func getRecentReceipts(limit: Int = 10) -> [Receipt] {
        let allReceipts = getSavedReceipts()
        return Array(allReceipts.prefix(limit))
    }

    /// Removes specified receipt from storage and notifies UI to refresh
    /// Allows users to clean up their receipt history and maintain organized data
