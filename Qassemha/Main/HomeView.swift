...
struct HomeView: View {
    ...
    /// Filters and combines real and example receipts based on selected tab
    /// For sent receipts, only shows those where current user is the admin
    ...
    /// Loads recent bill split activity from saved receipts
    /// Fetches receipts with split configurations and sorts by most recent
    ...
    /// Removes example receipts from the database if they were accidentally saved
    /// Runs only once to clean up demonstration data from previous app versions
...
struct RecentActivityCard: View {
    ...
    /// Calculates relative time string from configuration's last update
    /// Returns human-readable format like "Yesterday", "2 hours ago", etc.
    ...
    /// Determines the payment status for display
    /// Returns "completed" if all/user paid, "pending" otherwise
...
struct LiquidGlassAttributionView: View {
    ...
    /// Generates a randomized position near the base point for mesh gradient animation
    /// Creates fluid movement within a limited range to maintain gradient structure
