...
struct TransactionRow: View {
    ...
    /// Returns the appropriate SF Symbol icon for the transaction type
    /// Maps transaction types to visual indicators
    ...
    /// Determines the color scheme for the transaction type
    /// Green for incoming money, red for outgoing money
    ...
    /// Returns the prefix for amount display based on transaction type
    /// Shows + for credits and - for debits
...
struct AddFundsView: View {
    ...
    /// Validates if funds can be added with current inputs
    /// Checks for valid amount and selected payment method
    private var canAddFunds: Bool {
        guard let amountValue = Double(amount), amountValue > 0 else { return false }
        return selectedPaymentMethod != nil
    }

    /// Processes the add funds request through wallet manager
...
struct WithdrawFundsView: View {
    ...
    /// Validates if withdrawal can proceed with current inputs
    /// Checks for positive amount not exceeding wallet balance
    private var canWithdraw: Bool {
        guard let amountValue = Double(amount), amountValue > 0 else { return false }
        return amountValue <= walletManager.walletBalance
    }

    /// Processes the withdrawal request through wallet manager
    /// Validates balance and handles transaction result
...
struct PaymentMethodCard: View {
    ...
    /// Returns the appropriate icon for the payment method type
    /// Shows credit card or bank building based on method type
    private var icon: String {
        method.type == "card" ? "creditcard.fill" : "building.columns.fill"
    }

    /// Determines the brand-specific color for the payment method
    /// Maps card brands to their characteristic colors
...
struct PaymentMethodSelectionRow: View {
    ...
    /// Returns the appropriate icon for the payment method selection
    /// Shows credit card or bank building based on method type
    private var icon: String {
        method.type == "card" ? "creditcard.fill" : "building.columns.fill"
    }
...
struct AddPaymentMethodView: View {
    ...
    /// Validates if the payment method form is complete
    /// Checks required fields based on card or bank account type
    ...
    /// Creates and saves a new payment method from form inputs
    /// Extracts last four digits and detects card brand before saving
    ...
    /// Identifies Visa, Mastercard, Amex based on first digits
...
struct TransactionDetailView: View {
    ...
    /// Returns the appropriate icon for the transaction detail view
    /// Maps transaction types to corresponding SF Symbols
