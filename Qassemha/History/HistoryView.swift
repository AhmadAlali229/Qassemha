...
struct HistoryView: View {
    ...
    /// Loads and caches all transactions from receipts and wallet asynchronously
    /// Improves performance by loading heavy data on background thread
    ...
    /// Filters transactions based on search text, status, and date range
    /// Returns dynamically filtered list for display in the UI
    ...
    /// Groups transactions by date categories (Today, Yesterday, or specific dates)
    /// Provides organized display with user-friendly date labels
...
struct UnifiedTransaction: Identifiable {
    ...
    /// Returns color based on transaction status
    /// Provides visual indication of transaction state
    ...
    /// Returns appropriate SF Symbol icon for transaction type
    /// Provides visual distinction between receipts and wallet transactions
    var typeIcon: String {
        switch type {
...
struct AnalyticsSummaryCard: View {
    ...
    /// Calculates total amount spent from completed transactions
    /// Includes both receipt and payment type transactions
    private var totalSpent: Double {
        transactions.filter { ($0.type == .receipt || $0.category == "payment") && $0.status == "completed" }.reduce(0) { $0 + $1.total }
    }

    /// Calculates total amount received from payments and refunds
    /// Shows income from received transactions
    private var totalReceived: Double {
        transactions.filter { $0.category == "received" || $0.category == "refund" }.reduce(0) { $0 + $1.total }
    }

    /// Counts number of pending transactions
...
struct TransactionReportsView: View {
    ...
    /// Filters transactions based on selected report period
    /// Enables time-based analysis of spending patterns
    ...
    /// Returns transactions filtered for the selected time period
    /// Simplifies access to period-specific transaction data
    private var periodTransactions: [UnifiedTransaction] {
        filteredTransactionsByPeriod()
    }

    /// Calculates total spent in the selected period
    /// Used for period-specific financial analysis
    private var totalSpent: Double {
        periodTransactions.filter { ($0.type == .receipt || $0.category == "payment") && $0.status == "completed" }.reduce(0) { $0 + $1.total }
    }

    /// Calculates total received in the selected period
    /// Used for period-specific income analysis
    private var totalReceived: Double {
        periodTransactions.filter { $0.category == "received" || $0.category == "refund" }.reduce(0) { $0 + $1.total }
    }

    /// Calculates net amount (received minus spent) for the period
    /// Shows overall financial balance for the selected timeframe
    private var netAmount: Double {
        totalReceived - totalSpent
    }

    /// Groups transactions by category with totals and counts
    /// Enables category-based spending analysis
    ...
    /// Calculates average transaction value for the period
    /// Provides insight into typical spending amounts
    ...
    /// Groups transactions by week with total spending per week
    /// Creates weekly spending trend data for visualization
...
struct ExportOptionsView: View {
    ...
    /// Initiates export process for transactions in selected format
    /// Generates PDF or CSV file asynchronously on background thread
    ...
    /// Generates a formatted PDF report of transactions
    /// Creates professional document with transaction details
    ...
    /// Generates CSV file of transactions for spreadsheet export
    /// Creates comma-separated values file compatible with Excel/Sheets
...
struct ShareSheet: UIViewControllerRepresentable {
    ...
    /// Creates UIActivityViewController for sharing files
    /// Provides system share sheet for exporting documents
    func makeUIViewController(context: Context) -> UIActivityViewController {
        let controller = UIActivityViewController(activityItems: items, applicationActivities: nil)
        return controller
    }

    /// Updates the activity controller (no-op for this implementation)
    /// Required protocol method with no update logic needed
...
struct HistoryTransactionDetailView: View {
    ...
    /// Creates human-readable date string for display
