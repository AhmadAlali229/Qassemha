...
struct AllReceiptsView: View {
    ...
    /// Loads all saved receipts from CoreData
    /// Refreshes the receipts list to reflect the latest changes
    private func loadReceipts() {
        receipts = CoreDataManager.shared.getSavedReceipts()
        applyFiltersAndSort()
    }

    /// Applies search, user filtering, and sorting to the receipts list
    /// Ensures only relevant receipts are displayed based on current filters
