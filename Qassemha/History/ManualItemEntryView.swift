...
struct AddItemView: View {
    ...
    /// Calculates total price for the item being added/edited
    /// Displays real-time calculation in the UI
...
struct ManualReceiptItem: Identifiable, Equatable {
    ...
    /// Calculates total price by multiplying quantity and unit price
    /// Provides computed total for each item
    var totalPrice: Double {
        quantity * unitPrice
    }
}
...
class ManualEntryViewModel: ObservableObject {
    ...
    /// Calculates subtotal by summing all item prices
    /// Provides base amount before tax and tip
    var subtotal: Double {
        items.reduce(0) { $0 + $1.totalPrice }
    }

    /// Calculates grand total including tax and tip
    /// Returns final receipt amount
    var total: Double {
        subtotal + tax + tip
    }

    /// Adds a new item to the receipt
    /// Updates category suggestion based on added items
    func addItem(_ item: ManualReceiptItem) {
        items.append(item)
        updateCategorySuggestion()
    }

    /// Updates an existing item at specified index
    /// Refreshes category suggestion after update
    ...
    /// Removes item from receipt at specified index
    /// Updates category suggestion based on remaining items
    ...
    /// Auto-suggests receipt category based on store name and items
    /// Helps users categorize receipts without manual selection
    ...
    /// Determines receipt category by matching store name keywords
    /// Uses predefined keyword lists for intelligent categorization
    ...
    /// Maps item-level category to receipt-level category
    /// Enables category inference from item types
    ...
    /// Converts manual items to Receipt model and saves to CoreData
    /// Persists the manually entered receipt for later access
    func saveReceipt() {
