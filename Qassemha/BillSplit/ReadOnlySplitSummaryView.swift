...
struct ReadOnlySplitSummaryView: View {
    ...
    /// Initializes read-only split summary with receipt and configuration
    /// Sets up local state for tracking changes without persisting
    ...
    /// Generates split summaries from local configuration
    /// Uses local state instead of persisted configuration
    ...
    /// Handles payment initiation for a participant
    /// Shows item selection for non-equal splits, direct payment for equal split
    ...
    /// Handles payment completion after payment method selection
    /// Updates local payment status and refreshes wallet balance
