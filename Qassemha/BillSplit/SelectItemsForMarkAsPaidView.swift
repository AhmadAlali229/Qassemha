...
struct SelectItemsForMarkAsPaidView: View {
    ...
    /// Returns all items from the receipt
    /// For sent receipts, all items are available for selection
    private var assignedItems: [ReceiptItem] {
        // Show all items for sent receipts
        receipt.items
    }

    /// Returns set of item IDs already marked as paid by participant
    /// Used to pre-select items when reopening selection
...
struct MarkAsPaidItemCard: View {
    ...
    /// Excludes current participant from list
