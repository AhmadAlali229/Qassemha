...
struct CustomSplitOptionsView: View {
    ...
    /// Calculates the total percentage or amount currently assigned to all participants
    /// Returns sum of custom percentages for percentage splits or sum of custom amounts for fixed splits
    ...
    /// Calculates the remaining percentage or amount not yet assigned
    /// Returns difference between 100% and assigned for percentage, or total minus assigned for fixed amounts
    ...
    /// Validates if the split configuration is complete and correct
    /// Checks if assigned percentages equal 100% or assigned amounts equal receipt total
    var isValid: Bool {
        if configuration.splitType == .percentage {
    ...
    /// Distributes the total amount or 100% equally among all participants
    /// Creates or updates split entries with equal shares based on split type
    ...
    /// Resets all custom percentages or amounts to zero
    /// Allows users to start fresh with split assignments
    ...
    /// Assigns any remaining unassigned amount to the first participant
    /// Used for custom amount splits to quickly balance the total
...
struct CustomSplitParticipantRow: View {
    ...
    /// Calculates the monetary equivalent of the current value
    /// Converts percentage to dollar amount or returns the amount directly
