...
class BillSplitManager: ObservableObject {
    ...
    /// Creates new split configuration for a receipt with participants
    /// Sets current user as admin and initializes all assignments
    ...
    /// Retrieves existing split configuration for a receipt
    ...
    /// Updates existing split configuration and persists changes
    /// Broadcasts update notification to refresh UI
    ...
    /// Removes from storage and notifies observers
    ...
    /// Calculates equal split dividing total evenly among all participants
    /// Includes proportional tax and tip if configured
    ...
    /// Calculates split based on individual item assignments
    /// Each participant pays only for their assigned items with proportional tax/tip
    ...
    /// Calculates split based on percentage allocations
    /// Each participant pays their assigned percentage of the total
    ...
    /// Calculates split using custom amounts for each participant
    /// Allows specifying exact dollar amounts per person
    ...
    /// Main calculation dispatcher that routes to appropriate split method
    /// Determines which calculation method to use based on split type
    ...
    /// Assigns an item to specific participants
    /// Updates item assignment configuration
    ...
    /// Sets the split type for a specific item (equal, percentage, custom)
    /// Allows per-item split customization
    ...
    /// Sets custom split value for a participant on a specific item
    /// Used for percentage or custom amount splits
    ...
    /// Generates user-friendly split summaries for display
    /// Combines participant info with calculated amounts
    ...
    /// Toggles payment status between paid and unpaid
    /// Convenience method for marking payments
    ...
    /// Removes participant from paid list
    ...
    /// Validates split configuration and returns warning messages
    /// Checks for common issues like unassigned items or invalid percentages
