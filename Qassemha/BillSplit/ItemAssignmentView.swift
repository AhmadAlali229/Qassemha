...
struct ItemAssignmentView: View {
    ...
    /// Filters participants to show only the current user for item assignment
    /// Limits individual item splits to only the user's own items
    ...
    /// Retrieves or creates an item assignment for the specified receipt item
    /// Ensures every item has an assignment entry for tracking participant splits
    ...
    /// Toggles a participant's assignment to a specific item
    /// Adds or removes the participant from the item's split list
    ...
    /// Returns all receipt items assigned to a specific participant
    /// Filters items based on the participant's presence in item assignments
    ...
    /// Calculates the total amount a participant owes based on their assigned items
    /// Applies equal, percentage, or custom split rules for each assigned item
...
struct ItemAssignmentRow: View {
    ...
    /// Returns the list of participants assigned to this item
    /// Filters the full participant list to match assigned IDs
...
struct ParticipantAssignmentChip: View {
    ...
    /// Calculates the display text for the participant's share of the item
    /// Returns formatted dollar amount, percentage, or question mark if not set
