...
struct SplitSummaryView: View {
    ...
    /// Generates split summaries for all participants showing their owed amounts
    /// Calculates each person's share based on the selected split method
    var summaries: [SplitSummary] {
        manager.generateSplitSummaries(receipt: receipt, config: configuration)
    }

    /// Identifies if this is an example receipt that should not be saved
    /// Example receipts have special UUID prefixes for demonstration purposes
    ...
    /// Calculates the total paid and total amounts for the payment overview
    /// Returns tuple with sum of paid amounts and the full receipt total
    ...
    /// Resets the split configuration to its initial state
    /// Clears all participants and assignments, then dismisses the view
    ...
    /// Deletes the entire split configuration for this receipt
    /// Removes all split data and dismisses the view
    private func deleteSplit() {
        manager.deleteConfiguration(for: receipt.id)
        dismiss()
    }

    /// Toggles a participant's payment status between paid and unpaid
    /// Updates the configuration and persists changes
    ...
    /// Handles marking items as paid for a participant in "By Item" splits
    /// Updates paidItems dictionary with selected items or removes entry if none selected
    ...
    /// Processes payment completion result and updates participant status
    /// Marks participant as paid and refreshes wallet balances
    ...
    /// Schedules payment reminder notifications for all unpaid participants
    /// Uses the configured reminder schedule and due date from the split configuration
    ...
    /// Cancels all scheduled notifications for this bill split
    /// Removes reminders for all participants regardless of payment status
