...
struct BillSplitView: View {
    ...
    /// Initializes the view with a receipt and loads or creates its split configuration
    /// Ensures admin is set for sent receipts by finding or creating the "You" participant
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showingSplitSummary) {
            if receipt.receiptType == .sent {
                SentReceiptView(receipt: receipt, configuration: configuration)
            } else {
    ...
    /// Resets the bill split configuration to its initial state
    /// Clears all participants, assignments, and custom splits for this receipt
    private func resetSplit() {
        configuration = SplitConfiguration(receiptId: receipt.id)
        manager.updateConfiguration(configuration)
    }

    /// Ensures the current user is set as admin for sent receipts
    /// Creates or finds the "You" participant and assigns them as admin if not already set
